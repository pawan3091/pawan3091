...
def func(x,E,l0,m,N):
    ...
    for vi in v1:
        ...
        if vi>=small_e:
            ...
            omega1.append(0)
            k1.append(1j*np.sqrt(vi-small_e))
    ...
    for vi in v2:
        ...
        if vi>=small_e:
            ...
            omega2.append(0)
            k2.append(1j*np.sqrt(vi-small_e))
    ...
    #IC1=[np.exp(2**2),k1[0]*np.exp(2**2)]
    #IC2=[np.exp(1*2**2),-k2[0]*np.exp((2)*(2))]
    IC1=np.real(np.array([np.exp(1j*k1[0]*2),1j*k1[0]*np.exp(1j*k1[0]*2)]))
    IC2=-np.real(np.array([np.exp(1j*k2[0]*(-2)),-1j*k2[0]*np.exp(1j*k2[0]*(-2))]))

    res1=RK4(Sol1, IC1,-f,0,N,k1)
    res2=RK4(Sol1, IC2,f,0,N,k2)
...
def secant(e1,e2,tol):
    s1=func(x,e1,l0,m,N)[-2]
    s2=func(x,e2,l0,m,N)[-2]

    n=0
    while abs((e1-e2)/e2)>tol:
